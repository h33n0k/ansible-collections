...
help: ## Show help for each command
	...
	@echo -e "  \033[33mSCENARIO\033[0m=default             \033[2;37m# Specify the molecule scenario\033[0m"
	...
	@echo -e "  \033[33mBASE_BRANCH\033[0m=<branch>         \033[2;37m# Base branch to compare against for changes (default: HEAD)\033[0m"
		...
		if [ -d "$$roles_dir" ]; then \
			for role in "$$roles_dir"/*; do \
				...
				if [ -d "$$role" ]; then \
					...
					find "$$role" -type f -iname molecule.yml | while read -r scenario; do \
						...
						if [ "$(STAGED_ONLY)" = "true" ]; then \
							...
							changed=$$(git diff --name-only "$(CHANGED_SINCE)..$$base_branch" | grep "^$$role" || true); \
							if [ -z "$$changed" ]; then \
